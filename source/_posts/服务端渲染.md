...
服务端res.send()里面引入js增加
代码目录src/server/index.js
```js
<script>
	window.context = {
...
```
src/store/index.js
store改写，导出default getStore改写
```js
export const getClientStore = () => {
  const defaultState = window.context.state
  return createStore(reduser, defaultState, applyMiddleware(thunk))
}
```
## 6.1节开始记录
**axios 的 instance使用**
...
// 使用方法例子
instanceClient.get('/api/new.js').then().
```
**redux-thunk 的 withExtraArgument方法的使用**
```js
createStore(applyMiddleware(thunk.withExtraArgument(api))) // api可以带一个参数
// 用法
// action.js
const actionAxios = () => {
	return (action, getState, api) => {}
}
```

因为我的登陆接口是自己写的，没有遇到cookie这种问题，但是还是得记录一下
因为服务端请求的时候没有携带cookie所以，刷新页面的时候会出现登陆状态跳动的情况
解决
```js
//src/client/request.js
// 这里代码不是最终版，要套到我的文件里面还得改一下，不过原理就是这样
// 上层把req传过来就行了
export const instanceServer = req => axios.create({
...
```
还有loadData，必须返回promise，但是我没有返回也是可以用的，得研究一下

接触到路由一个新的方法，Redirect, 重定向
```js
...
## 查错工具
**beyond compare** 可以比对两个文件之间是否一致


## 服务端渲染  componentwillmount useEffect
服务端渲染会执行componentwillmount 但不会执行useEffect
目前尚不知，是否有替代方法，或者是否必须使用componentwillmount

## 第8节
这章很水，介绍了服务端渲染css，很简单，高阶组件(抽离代码公共部分),不知道这个是否和hooks抽离相同，如果是的话，hooks抽离就太简单了

## 9章
**react-helme**  定制每个页面的title，和Description
